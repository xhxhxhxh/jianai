...
Page({
      ...
      if(res.error === 0) {
        ...
        this.setData(data, async () => {
          if(addToTop) {
            ...
            this.setData({
              scrollTop: res.height - scrollBottom,
  ...
  endTouch(e) {
    ...
    let { touch_down, inner_height } = this;
    let start_scroll = ''
    // 获取 scroll-view 的高度和当前的 scrollTop 位置
    wx.createSelectorQuery().select('.chat-area').fields({  
      scrollOffset: true, 
      size: true 
    }, (rect) => { 
      start_scroll = rect.scrollTop <= 0 ? -40 : rect.scrollTop;
      scrollBottom = inner_height - start_scroll
